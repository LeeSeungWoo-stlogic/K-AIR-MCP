...
행 상한은 `MCP_ROW_LIMIT`(기본 10000). `filters.op`: eq, ne, gt, gte, lt, lte, like, in, is_null, is_not_null. PG 직조회는 `/meta/table`의 컬럼 형(date, timestamp, numeric 등)에 맞춰 필터 값을 서버에서 변환한다.
